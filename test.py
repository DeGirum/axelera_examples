...
if __name__ == "__main__":
    try:
        ...
        if "AXELERA/METIS" in supported_devices:
            device_type = "AXELERA/METIS"
        else:
            print(
                "Axelera device is NOT supported or NOT recognized properly. Please check the installation."
            )
            sys.exit(1)

        print(f"Using device type: {device_type}")

        print("Running inference on Axelera device")
